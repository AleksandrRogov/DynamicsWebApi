...
class RequestUtility {
     ...
     */
    static composeUrl(request, functionName, joinSymbol, config, url) {
        var headers = {};
        var requestArray = [];
        ...
        if (request) {
            ...
            if (request.savedQuery) {
                requestArray.push("savedQuery=" + ErrorHelper_1.ErrorHelper.guidParameterCheck(request.savedQuery, 'DynamicsWebApi.' + functionName, "request.savedQuery"));
            }
            if (request.userQuery) {
                requestArray.push("userQuery=" + ErrorHelper_1.ErrorHelper.guidParameterCheck(request.userQuery, 'DynamicsWebApi.' + functionName, "request.userQuery"));
            }
            if (request.count) {
                ...
                if (typeof request.expand === 'string') {
                    ...
                    for (var i = 0; i < request.expand.length; i++) {
                        if (request.expand[i].property) {
                            var expandConverted = RequestUtility.composeUrl(request.expand[i], functionName + " $expand", ";");
