...
class RequestUtility {
        ...
        if (request) {
            if (request.navigationProperty) {
                ...
                if (request.navigationPropertyKey) {
                    var navigationKey = ErrorHelper_1.ErrorHelper.keyParameterCheck(request.navigationPropertyKey, `DynamicsWebApi.${functionName}`, "request.navigationPropertyKey");
                    url += "(" + navigationKey + ")";
                }
                ...
                if (typeof request.expand === "string") {
                    ...
                    for (var i = 0; i < request.expand.length; i++) {
                        if (request.expand[i].property) {
                            var expandConverted = RequestUtility.composeUrl(request.expand[i], `${functionName} $expand`, config, ";");
                            if (expandConverted) {
    ...
    static convertToBatch(requests, config) {
        var batchBoundary = `dwa_batch_${Utility_1.Utility.generateUUID()}`;
        var batchBody = [];
