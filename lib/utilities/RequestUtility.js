...
class RequestUtility {
     ...
     */
    static compose(request, config, functionName) {
        let path = "";
        if (!request.url) {
            if (!request._isUnboundRequest && !request.collection) {
                ErrorHelper_1.ErrorHelper.parameterCheck(request.collection, `DynamicsWebApi.${functionName}`, "request.collection");
            }
            if (request.collection != null) {
                ErrorHelper_1.ErrorHelper.stringParameterCheck(request.collection, `DynamicsWebApi.${functionName}`, "request.collection");
                path = request.collection;
                if (request.contentId) {
                    ErrorHelper_1.ErrorHelper.stringParameterCheck(request.contentId, `DynamicsWebApi.${functionName}`, "request.contentId");
                ...
                if (request.key) {
                    request.key = ErrorHelper_1.ErrorHelper.keyParameterCheck(request.key, `DynamicsWebApi.${functionName}`, "request.key");
                }
                else if (request.id) {
                    request.key = ErrorHelper_1.ErrorHelper.guidParameterCheck(request.id, `DynamicsWebApi.${functionName}`, "request.id");
                }
                if (request.key) {
                    path += `(${request.key})`;
                }
            ...
            if (request.fetchXml) {
                ErrorHelper_1.ErrorHelper.stringParameterCheck(request.fetchXml, `DynamicsWebApi.${functionName}`, "request.fetchXml");
                let join = path.indexOf("?") === -1 ? "?" : "&";
                path += `${join}fetchXml=${encodeURIComponent(request.fetchXml)}`;
            }
        }
        else {
            ErrorHelper_1.ErrorHelper.stringParameterCheck(request.url, `DynamicsWebApi.${functionName}`, "request.url");
            path = request.url.replace(config.webApiUrl, "");
            path = RequestUtility.composeUrl(request, functionName, config, "&", path);
        }
        let async = true;
        if (request.hasOwnProperty("async") && request.async != null) {
            ErrorHelper_1.ErrorHelper.boolParameterCheck(request.async, `DynamicsWebApi.${functionName}`, "request.async");
            async = request.async;
        }
     ...
     */
    static composeUrl(request, functionName, config, joinSymbol, url) {
        let queryArray = [];
        joinSymbol = joinSymbol || "&";
        url = url || "";
        if (request) {
            if (request.navigationProperty) {
                ErrorHelper_1.ErrorHelper.stringParameterCheck(request.navigationProperty, `DynamicsWebApi.${functionName}`, "request.navigationProperty");
                url += "/" + request.navigationProperty;
                if (request.navigationPropertyKey) {
                    let navigationKey = ErrorHelper_1.ErrorHelper.keyParameterCheck(request.navigationPropertyKey, `DynamicsWebApi.${functionName}`, "request.navigationPropertyKey");
                    url += "(" + navigationKey + ")";
                }
                if (request.navigationProperty === "Attributes") {
                    if (request.metadataAttributeType) {
                        ErrorHelper_1.ErrorHelper.stringParameterCheck(request.metadataAttributeType, `DynamicsWebApi.${functionName}`, "request.metadataAttributeType");
                        url += "/" + request.metadataAttributeType;
                    }
                }
            }
            if (request.select != null && request.select.length) {
                ErrorHelper_1.ErrorHelper.arrayParameterCheck(request.select, `DynamicsWebApi.${functionName}`, "request.select");
            ...
            if (request.filter) {
                ErrorHelper_1.ErrorHelper.stringParameterCheck(request.filter, `DynamicsWebApi.${functionName}`, "request.filter");
            ...
            if (request.savedQuery) {
                queryArray.push("savedQuery=" + ErrorHelper_1.ErrorHelper.guidParameterCheck(request.savedQuery, `DynamicsWebApi.${functionName}`, "request.savedQuery"));
            }
            if (request.userQuery) {
                queryArray.push("userQuery=" + ErrorHelper_1.ErrorHelper.guidParameterCheck(request.userQuery, `DynamicsWebApi.${functionName}`, "request.userQuery"));
            }
            if (request.apply) {
                ErrorHelper_1.ErrorHelper.stringParameterCheck(request.apply, `DynamicsWebApi.${functionName}`, "request.apply");
                queryArray.push("$apply=" + request.apply);
            }
            if (request.count) {
                ErrorHelper_1.ErrorHelper.boolParameterCheck(request.count, `DynamicsWebApi.${functionName}`, "request.count");
                queryArray.push("$count=" + request.count);
            }
            if (request.top && request.top > 0) {
                ErrorHelper_1.ErrorHelper.numberParameterCheck(request.top, `DynamicsWebApi.${functionName}`, "request.top");
                queryArray.push("$top=" + request.top);
            }
            if (request.orderBy != null && request.orderBy.length) {
                ErrorHelper_1.ErrorHelper.arrayParameterCheck(request.orderBy, `DynamicsWebApi.${functionName}`, "request.orderBy");
                queryArray.push("$orderby=" + request.orderBy.join(","));
            }
            if (request.entity) {
                ErrorHelper_1.ErrorHelper.parameterCheck(request.entity, `DynamicsWebApi.${functionName}`, "request.entity");
            }
            if (request.data) {
                ErrorHelper_1.ErrorHelper.parameterCheck(request.data, `DynamicsWebApi.${functionName}`, "request.data");
            }
            if (request.isBatch) {
                ErrorHelper_1.ErrorHelper.boolParameterCheck(request.isBatch, `DynamicsWebApi.${functionName}`, "request.isBatch");
            }
            if (request.timeout) {
                ErrorHelper_1.ErrorHelper.numberParameterCheck(request.timeout, `DynamicsWebApi.${functionName}`, "request.timeout");
            }
            if (request.expand && request.expand.length) {
                ErrorHelper_1.ErrorHelper.stringOrArrayParameterCheck(request.expand, `DynamicsWebApi.${functionName}`, "request.expand");
                if (typeof request.expand === "string") {
                    ...
                    for (let i = 0; i < request.expand.length; i++) {
                        if (request.expand[i].property) {
                            let expandConverted = RequestUtility.composeUrl(request.expand[i], `${functionName} $expand`, config, ";");
                            if (expandConverted) {
    ...
    static composeHeaders(request, functionName, config) {
        let headers = {};
        ...
        if (request.ifmatch) {
            ErrorHelper_1.ErrorHelper.stringParameterCheck(request.ifmatch, `DynamicsWebApi.${functionName}`, "request.ifmatch");
            headers["If-Match"] = request.ifmatch;
        }
        if (request.ifnonematch) {
            ErrorHelper_1.ErrorHelper.stringParameterCheck(request.ifnonematch, `DynamicsWebApi.${functionName}`, "request.ifnonematch");
            headers["If-None-Match"] = request.ifnonematch;
        }
        if (request.impersonate) {
            ErrorHelper_1.ErrorHelper.stringParameterCheck(request.impersonate, `DynamicsWebApi.${functionName}`, "request.impersonate");
            headers["MSCRMCallerID"] = ErrorHelper_1.ErrorHelper.guidParameterCheck(request.impersonate, `DynamicsWebApi.${functionName}`, "request.impersonate");
        }
        if (request.token) {
            ErrorHelper_1.ErrorHelper.stringParameterCheck(request.token, `DynamicsWebApi.${functionName}`, "request.token");
            headers["Authorization"] = "Bearer " + request.token;
        }
        if (request.duplicateDetection) {
            ErrorHelper_1.ErrorHelper.boolParameterCheck(request.duplicateDetection, `DynamicsWebApi.${functionName}`, "request.duplicateDetection");
            headers["MSCRM.SuppressDuplicateDetection"] = "false";
        }
        if (request.noCache) {
            ErrorHelper_1.ErrorHelper.boolParameterCheck(request.noCache, `DynamicsWebApi.${functionName}`, "request.noCache");
            headers["Cache-Control"] = "no-cache";
        }
        if (request.mergeLabels) {
            ErrorHelper_1.ErrorHelper.boolParameterCheck(request.mergeLabels, `DynamicsWebApi.${functionName}`, "request.mergeLabels");
    ...
    static composePreferHeader(request, functionName, config) {
        ...
        if (request.prefer && request.prefer.length) {
            ErrorHelper_1.ErrorHelper.stringOrArrayParameterCheck(request.prefer, `DynamicsWebApi.${functionName}`, "request.prefer");
        ...
        if (returnRepresentation) {
            ErrorHelper_1.ErrorHelper.boolParameterCheck(returnRepresentation, `DynamicsWebApi.${functionName}`, "request.returnRepresentation");
            prefer.push("return=representation");
        }
        if (includeAnnotations) {
            ErrorHelper_1.ErrorHelper.stringParameterCheck(includeAnnotations, `DynamicsWebApi.${functionName}`, "request.includeAnnotations");
            prefer.push(`odata.include-annotations="${includeAnnotations}"`);
        }
        if (maxPageSize && maxPageSize > 0) {
            ErrorHelper_1.ErrorHelper.numberParameterCheck(maxPageSize, `DynamicsWebApi.${functionName}`, "request.maxPageSize");
            prefer.push("odata.maxpagesize=" + maxPageSize);
        }
        if (trackChanges) {
            ErrorHelper_1.ErrorHelper.boolParameterCheck(trackChanges, `DynamicsWebApi.${functionName}`, "request.trackChanges");
    ...
    static stringifyData(data, config) {
        let stringifiedData;
        if (data) {
